...
try {
    ...
    console.warn('[headless-util] Puppeteer not available:', error.message);
    puppeteer = null;
}
...
class PuppeteerHelper {
    ...
    constructor() {
        ...
        if (!puppeteerAvailable) {
            console.warn('[headless-util] Puppeteer is not installed. Install it with: npm install puppeteer');
        }
    }
    ...
    _checkAvailability() {
        if (!puppeteerAvailable) {
            throw new Error('Puppeteer is not available. Please install it with: npm install puppeteer');
        }
    }
    ...
    async gotoHtml(config) {
        this._checkAvailability(); // 检查puppeteer是否可用
...
// 导出PuppeteerHelper类和可用性状态
export const puppeteerHelper = PuppeteerHelper
export const isPuppeteerAvailable = puppeteerAvailable