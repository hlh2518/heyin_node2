...
export default (fastify, options, done) => {
    ...
    fastify.get('/', {preHandler: validateBasicAuth}, async (request, reply) => {
        ...
        for (const file of files) {
            if (/^HEYINS\.md$/i.test(file)) {
                HEYINSPath = path.join(options.rootDir, file);
                break;
            }
        }

        // 如果未找到 HEYINS.md 文件
        if (!HEYINSPath && !process.env.VERCEL) {
            let fileHtml = files.map(file => `<li>${file}</li>`).join('');
            return reply.code(404).type('text/html;charset=utf-8').send(`<h1>HEYINS.md not found</h1><ul>${fileHtml}</ul>`);
        } else if (!HEYINSPath && process.env.VERCEL) {
            const tmpIndexHtml = readFileSync(indexHtmlPath, 'utf-8');
            return reply.type('text/html;charset=utf-8').send(tmpIndexHtml);
        }

        // 读取 HEYINS.md 文件内容
        const markdownContent = readFileSync(HEYINSPath, 'utf-8');
        ...
        if (!existsSync(indexHtmlPath)) {
            console.log(`将HEYINS.md 本地文件:${indexHtmlPath}`);
            ...
            if (indexHtmlHash !== tmpIndexHtmlHash) {
                console.log(`HEYINS.md发生了改变，更新本地文件:${indexHtmlPath}`);
                writeFileSync(indexHtmlPath, indexHtml, 'utf8');
            }
        }
